...
"""
"""
Physics-Informed LSTM (PI LSTM) Surrogate Model (Inequality MB-based) for Street-Scale Flood Forecasting in Norfolk, VA
...
'define run_name'
trial_model='LSTM_GPU_44_Inequal'
trial_domain ='S_22'
trial_seed ='seed1_es1'
trial_lambda = 'lam_opt_ab_hp'
trial_data ='TUFLOW_max_true'
...
'because, n_ahead = 5, so need to delete RH, TD_HR'    
train_x = train_x[:, :, :-2]
...
def physics_informed_loss(y_true, y_pred, train_x_raw, mean_wdepth, std_wdepth, mean_wdepth_vol, std_wdepth_vol, lambda_phy, lambda_data):
    ...
    y_true = tf.reshape(y_true, (-1, 5, 2))
    ...
    data_loss_depth = tf.reduce_mean(tf.square(pred_depth - true_depth[:, :-1]))
    data_loss_volume = tf.reduce_mean(tf.square((pred_volume - true_volume[:, :-1])))
    ...
    for i in range(4):
        'Constraint 1'
        rain_vol_current = train_x_raw[:, 3 , 5+(2*i)]
        inflow_vol_current = train_x_raw[:, 3 , 5+(2*i)+1]
        ...
        if i == 0:
            ...
            depth_true_vol_prev = y_true_vol_unscaled[:, i - 1]
            
        area = train_x_raw[:, 0, 2]

        lhs = (depth_pred_vol_current - depth_true_vol_prev)
        rhs = (rain_vol_current + inflow_vol_current)
        
        residual1 = tf.nn.relu((lhs - rhs)/area)
        residual1_norm = residual1 / std_wdepth
        physics_losses.append(tf.square(residual1_norm))

        'Constraint 2'
        rain_vol_next = train_x_raw[:, 3 , 7+(2*i)]
        inflow_vol_next = train_x_raw[:, 3 , 7+(2*i)+1]
        
        depth_true_vol_next = y_true_vol_unscaled[:, i + 1] 

        lhs = (depth_true_vol_next - depth_pred_vol_current)
        rhs = (rain_vol_next + inflow_vol_next)
        
        residual2 = tf.nn.relu((lhs - rhs)/area)
        residual2_norm = residual2 / std_wdepth 
        physics_losses.append(tf.square(residual2_norm))
...
def run_model(hp, return_model=False):
    ...
    validation_y_wd = validation_y[:, :4 , 0]
